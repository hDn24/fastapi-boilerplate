...
from app.api.models import user as models

# import schemas
from app.api.schemas import post as schemas
from app.database import get_db

router = APIRouter(prefix="/posts", tags=["posts"])


@router.get("/", response_model=List[schemas.CreatePost], dependencies=[Depends(get_db)])
...
def get_test_one_post(id: int, db: Session = Depends(get_db)):
    ...
    if idv_post is None:
        raise HTTPException(
            status_code=status.HTTP_400_BAD_REQUEST, detail=f"The id: {id} you requested for does not exist"
        )
    return idv_post
